...
In this practical we're going to have a closer look at these techniques, introduce a more advanced technique called topic modeling.
...
This is because this is useful knowledge.
...
vaccin_2021_04_08 <- read_csv("Desktop/(2.1) Exploratory text analysis/vaccin_2021_04_08.csv")
View(vaccin_2021_04_08)
```
...
vaccin_2021_04_08 <- read_delim("Desktop/(2.1) Exploratory text analysis/vaccin_2021_04_08.csv", 
...
```{r, eval=F}
files = list.files('Desktop/(2.1) Exploratory text analysis', pattern = 'vaccin', full.names = T)
...
One thing you'll notice is that words will look weird, because stemming will sometimes cut-off the suffix (last part of the word).
...
> **Question 1.a**. Create a wordcloud of this data with the `textplot_wordcloud` function, as you did last week. Use arguments (e.g., max_words, color) to make it a nice wordcloud. Paste the wordcloud in your answer document, and provide a brief interpretation of what the wordcloud tells you about the data.

```{r, eval=F}
textplot_wordcloud(dtm, max_words = 50, color = c('orange','purple')) ## change colors
...
```{r, eval=F}
is_negative <- docvars(dtm)$Sentiment == 'negative' 
negative_dtm <- dtm[is_negative,]
textplot_wordcloud(negative_dtm, max_words = 25)

ts <- textstat_keyness(dtm, is_negative)
...
```{r}
textplot_wordcloud(hashtag_dtm, max_words = 50, color = c('orange','purple')) ## change colors
textstat_frequency(hashtag_dtm, n = 10)    
                                                                                                          
```
                                                                                                             > **Question 1.d**. Answer question 1.b again (keyness of negative tweets + interpretation), but this time using the hashtag dtm. 
                                                                                                                
```{r, eval=F}
is_negativeHT <- docvars(hashtag_dtm)$Sentiment == 'negative' 
negativeHT_dtm <- hashtag_dtm[is_negativeHT,]
textplot_wordcloud(negativeHT_dtm, max_words = 25)

ts <- textstat_keyness(hashtag_dtm, is_negativeHT)
head(ts, 20)    ## view first 20 results
