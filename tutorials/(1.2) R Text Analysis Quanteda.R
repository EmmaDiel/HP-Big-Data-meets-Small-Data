...
The first step is getting text into R in a proper format. Text is often stored in a variety of formats, from plain text and CSV files to HTML and PDF, and more often than not with different 'encodings'. There are various packages for reading these file formats, and there is also the convenient [readtext](https://cran.r-project.org/web/packages/readtext/vignettes/readtext_vignette.html) that is specialized for reading texts from a variety of formats.
...
However, directly converting a text to a DTM is a bit crude. Note, for instance, that the words `cats`, and `cat` are given different columns.
In this DTM, "cats" and "awesome" are as different as "cats" and "cat", but for many types of analysis we would be more interested in the fact that both texts are about felines, and not about the specific word that is used. Also, for performance it can be useful (or even necessary) to use fewer columns, and to ignore less interesting words such as `is` or very rare words such as `soliloquy`.
...
This dtm has 23,469 documents and 16,032 features (i.e. terms), and no longer shows the actual matrix because it simply wouldn't fit. Depending on the type of analysis that you want to conduct, we might not need this many words, or might actually run into computational limitations. 

Luckily, many of these 16K features are not that informative. The distribution of term frequencies tends to have a very long tail, with many words occurring only once or a few times in our corpus. For many types of bag-of-words analysis it would not harm to remove these words, and it might actually improve results. 
...
**Exercise 1:** Create two subset of the dtm. The first one should only contain speeches of "Abraham Lincoln" and the second of "George W. Bush". In a first step, create two wordclouds. How did these president differ with regard to the words they used? Second, create a keyness plot to visualize the difference between Bush and the rest of the entire corpus
...
You can perform a basic dictionary search. In terms of query options this is less advanced than other packages, but quanteda offers more ways to analyse the dictionary results. Also, it supports the use of existing dictionaries, for instance for sentiment or topic analyses (but mostly for english dictionaries, we will do this in the next practical session).
...
**Exercise 2:** Create a barplot that compares how much each president has talked about the four topics (freedom, military, economy, terrorism). In how far does George Bush (Jr) differ from the others? Tip: The following code already converts the original corpus into a data.frame, merges it with the data frame we just created, and summarized the topics per president. You now may have to engage in some reshaping before you can plot the data in a meaningful way. Also: coord_flip() may be good to use...
